...
function autoTheme(hex){return{color:hex,bg:lighten(hex,0.82),text:darken(hex,0.45)};}
...
function App(){
  ...
  function addRepEvts(base,cat,note,h,half,dur,repType,cd,startDate){
    ...
    while(d<=end){
      if(rd.includes(d.getDay())){
        const dk=dateKey(d);
        if(!out[dk]) out[dk]=[];
        out[dk]=[...out[dk],{id:Date.now()+'_'+Math.random(),cat,note,h,half,dur,done:false,repId}];
      }
      d=addDays(d,1);
    }
    return out;
  ...
  function addFromForm(){
    ...
    if(form.rep==='none'){
      if(!ne[dk]) ne[dk]=[];
      ne[dk]=[...ne[dk],{id:Date.now(),cat:form.cat,note:form.note,h:hi,half,dur:form.dur,done:false}];
    } else {
      ne=addRepEvts(ne,form.cat,form.note,hi,half,form.dur,form.rep,form.repDays,cursor);
    }
    setForm(f=>({...f,note:''}));
    setEvts(ne);
  }
  ...
  function saveModal(){
    ...
    let ne={...events};
    if(modal.evtId){
      ne[modal.dk]=(ne[modal.dk]||[]).map(e=>String(e.id)===modal.evtId?{...e,cat:modal.cat,note:modal.note,h:modal.h,half:modal.half,dur:modal.dur,customColor:uc?modal.color:null,customTheme:theme}:e);
    } else {
      if(modal.rep==='none'){
        if(!ne[modal.dk]) ne[modal.dk]=[];
        ne[modal.dk]=[...ne[modal.dk],{id:Date.now(),cat:modal.cat,note:modal.note,h:modal.h,half:modal.half,dur:modal.dur,done:false,customColor:uc?modal.color:null,customTheme:theme}];
      } else {
        ne=addRepEvts(ne,modal.cat,modal.note,modal.h,modal.half,modal.dur,modal.rep,modal.repDays,cursor);
      }
    }
  ...
  function handleDrop(dk,h,half){
    ...
    if(dragEvt.type==='new'){
      if(!ne[dk]) ne[dk]=[];
      ne[dk]=[...ne[dk],{id:Date.now(),cat:dragEvt.catId,note:'',h,half,dur:2,done:false}];
    } else {
  ...
  function EvBlock({ev,dk}){
    const th=ev.customTheme||catById(ev.cat),top=slotIdx(ev.h,ev.half||false)*SH,height=ev.dur*SH-2;
    ...
    return React.createElement('div',{
      ...
      style:{position:'absolute',left:2,right:2,top,height,borderRadius:4,padding:'3px 5px',cursor:'grab',zIndex:2,overflow:'hidden',display:'flex',flexDirection:'column',justifyContent:'space-between',background:th.bg,color:th.text,borderLeft:`3px solid ${th.color}`,opacity:ev.done?0.45:1}
    },
      React.createElement('div',{style:{fontSize:10,fontWeight:500,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',lineHeight:1.3}},catById(ev.cat).name+(ev.note?` · ${ev.note}`:''+(ev.repId?' ↻':''))),
      React.createElement('div',{style:{fontSize:9,opacity:0.7}},fmtH(ev.h,ev.half||false)+' · '+dl),
      height>32&&React.createElement('div',{style:{display:'flex',gap:2,marginTop:1}},
        [['✓',()=>toggleDone(dk,ev.id),ev.done?th.color+'44':'rgba(0,0,0,0.12)'],
         ['✎',()=>setModal({dk,evtId:String(ev.id),cat:ev.cat,note:ev.note||'',h:ev.h,half:ev.half||false,dur:ev.dur,color:ev.customColor||catById(ev.cat).color,rep:'none',repDays:[false,false,false,false,false,false,false]}),'rgba(0,0,0,0.12)'],
         ['✕',()=>delEvt(dk,ev.id),'rgba(0,0,0,0.12)']
        ].map(([ico,fn,bg])=>React.createElement('button',{key:ico,onClick:fn,style:{width:14,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,display:'flex',alignItems:'center',justifyContent:'center',background:bg,color:'inherit',padding:0,flexShrink:0}},ico))
      )
    );
  }
  ...
  function DayCol({dk}){
    ...
    return React.createElement('div',{
      ...
      ...SLOTS.map((s,i)=>React.createElement('div',{
        ...
        onClick:()=>!dragEvt&&setModal({dk,evtId:null,cat:'platzi',note:'',h:s.h,half:s.half,dur:2,color:'#1D9E75',rep:'none',repDays:[false,false,false,false,false,false,false]})
      })),
    ...
    React.createElement('div',{style:{display:'grid',gridTemplateColumns:'160px 1fr',gap:10}},
      ...
      React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:5}},
        ...
        ...cats.map(c=>React.createElement('div',{key:c.id,draggable:true,onDragStart:()=>setDragEvt({type:'new',catId:c.id}),
          onClick:()=>setModal({dk:dateKey(cursor),evtId:null,cat:c.id,note:'',h:8,half:false,dur:2,color:c.color,rep:'none',repDays:[false,false,false,false,false,false,false]}),
